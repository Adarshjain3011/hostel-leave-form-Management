...
async function fetchStudentsDetailUsingProgramAndBranch(text:any, college:any, programe:any, branch:any, hostel:any) {

    try {

        const excludedRoles = ['Admin', 'Principal', 'Gatekeeper', 'Warden'];

        // Find users with the given text in fullName or email, excluding certain roles
        ...
        // Filter userDetails based on programe, college, branch, and hostel

        const studentDetails = userDetails.filter(data => (
            data.refId.programe === programe &&
            data.refId.college === college &&
            data.refId.branch === branch &&
            (text ? (
                data.refId.enrollmentNo.includes(text) // Assuming enrollmentNo is a field in refId
                // data.refId.someOtherField.includes(text) // Check other fields in refId if needed
            ) : true) &&
            (hostel ? data.refId.hostel === hostel : true) // Check hostel if provided
        ));

    
        console.log("Filtered Student Details:", studentDetails);

        return studentDetails; // Return the filtered student details

    } catch (error:any) {

        console.error(`Failed to retrieve students details: ${error.message}`);
