...
import { ROLE,STATUS } from "@repo/common/config";
...
async function handler1 (req: CustomNextRequest) {
    try {
...
async function handler2(req: CustomNextRequest) {
 ...
 async function handler3(req: CustomNextRequest) {
    try {
        const body = await req.json();
        await middleware(req);
        const userId = req.user;
 ...
 async function handler4(req: CustomNextRequest, ) {
    try {
        await middleware(req);
...
export const GET = allowCors(handler1);
export const PATCH = allowCors(handler2);
export const POST = allowCors(handler3);
export const PUT = allowCors(handler4);
